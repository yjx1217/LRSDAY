...
perl $LRSDAY_HOME/scripts/parse_REannotate_gff.pl -p $prefix
...
$blast_dir/blastn -task blastn -query $TY2_query -num_threads 1 -db $db_tag -outfmt 7 >$prefix.$db_tag.blastn.fmt7.out
...
$blast_dir/makeblastdb  -in $db  -dbtype nucl -title $db_tag -hash_index -out $db_tag
$blast_dir/blastn  -task blastn -query $LTR_query -num_threads 4 -db $db_tag -outfmt 7 >$prefix.$db_tag.soloLTR.blastn.fmt7.out
