...
source ./../../env.sh
PATH=$bwa_dir:$samtools_dir:$gnuplot_dir:$mummer4_dir:$spades_dir:$prodigal_dir:$PATH
...
then
    ...
    $cap3_dir/cap3 $prefix.assembly.mt_contig.break_by_blast.fa -k 0 
	...
	cat $prefix.assembly.mt_contig.break_by_blast.fa.cap.contigs $prefix.assembly.mt_contig.break_by_blast.fa.cap.singlets > $prefix.assembly.mt_contig.break_by_blast.fa.cap.contigs_singlets
        cat $prefix.assembly.mt_contig.break_by_blast.fa.cap.contigs_singlets| sed "s/>/>chrMT_/" > $prefix.assembly.mt_contig.for_clean.fa
    fi
    ...
    $ragtag_dir/ragtag.py scaffold -t $threads -f 1 -u -o ${prefix}_chrMT_ragtag_out ref.chrMT.fa $prefix.assembly.mt_contig.circlator.fasta
    cat ${prefix}_chrMT_ragtag_out/ragtag.scaffold.fasta |sed "s/>chrMT_RagTag/>${prefix}.chrMT/"|sed -r "s/N{100}//gi" >  ${prefix}.assembly.mt_contig.ragtag.fa
    ...
    cat $prefix.assembly.primary_contig.fa $prefix.assembly.mt_contig.ragtag.fa $prefix.assembly.non_primary_contig.fa > $prefix.assembly.mt_improved.fa
    # generate dotplot for the mitochondrial genome only
    $mummer4_dir/nucmer -t $threads --maxmatch --nosimplify  -p $prefix.assembly.mt_improved.chrMT ref.chrMT.fa $prefix.assembly.mt_contig.ragtag.fa
...
then
    ...
    if [ -d "${prefix}_chrMT_ragtag_out" ]
    then
	rm -r ${prefix}_chrMT_ragtag_out
    fi
fi
