...
then
    ...
    if [[ $(egrep -c "^>" "$prefix.mt_contig.break_by_blast.fa.cap.contigs") -eq 0 ]]
    then
        cat $prefix.mt_contig.break_by_blast.fa | sed "s/>/>chrMT_/" > $prefix.mt_contig.for_clean.fa
    else
        cat $prefix.mt_contig.break_by_blast.fa.cap.contigs| sed "s/>/>chrMT_/" > $prefix.mt_contig.for_clean.fa
    fi

    $circlator_dir/circlator clean --verbose $prefix.mt_contig.for_clean.fa  ${prefix}.mt_contig.for_fixstart.fa 
    mv ${prefix}.mt_contig.for_fixstart.fa.fasta ${prefix}.mt_contig.for_fixstart.fa
    ...
    $circlator_dir/circlator fixstart --verbose --genes_fa $gene_start --min_id 90 --ignore $prefix.for_fixstart.for_skip.list $prefix.mt_contig.for_fixstart.fa  ${prefix}.mt_contig.circlator
