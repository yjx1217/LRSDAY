...
query="$LRSDAY_HOME/data/Yprime12clusters_header-renamed.fasta"
...
$ucsc_dir/pslCDnaFilter  -minId=0.9 -minAlnSize=1000 -bestOverlap -filterWeirdOverlapped   $prefix.$feature_type.blat.psl  $prefix.$feature_type.blat.filtered.by_query.psl
# Resolve target-coordinate overlaps across the different Y-prime queries.
perl $LRSDAY_HOME/scripts/select_best_nonoverlapping_psl.pl -i $prefix.$feature_type.blat.filtered.by_query.psl -o $prefix.$feature_type.blat.filtered.psl
$ucsc_dir/pslScore  $prefix.$feature_type.blat.filtered.psl | sort -nk5 -r > $prefix.$feature_type.blat.filtered.pslScore.out
perl $LRSDAY_HOME/scripts/psl2gff3.pl -i $prefix.$feature_type.blat.filtered.psl -o $prefix.$feature_type.raw.gff3 -ft $feature_type -t $prefix -r $prefix.assembly.nuclear_genome.tidy.fa -l $length_cutoff_for_completeness --query_as_parent --query_as_feature_suffix
perl $LRSDAY_HOME/scripts/tidy_maker_gff3.pl -i $prefix.$feature_type.raw.gff3 -r $prefix.assembly.nuclear_genome.tidy.fa -o $prefix.nuclear_genome.$feature_type.gff3 -t $prefix --preserve_parent
...
then
    ...
    rm $prefix.$feature_type.blat.psl
    rm $prefix.$feature_type.blat.filtered.by_query.psl
    rm $prefix.$feature_type.blat.filtered.psl
    rm $prefix.$feature_type.blat.filtered.pslScore.out
